...
const GLOSS = { 'réu': 'réu = a Justiça aceitou a denúncia e há ação penal em curso; não é condenação.', 'condenado': 'condenado = há decisão judicial condenatória; veja a instância e os recursos na descrição.', 'inquérito': 'inquérito = investigação formal em andamento; não é acusação nem condenação.', 'denúncia pendente': 'a PGR apresentou acusação; a Justiça ainda não decidiu se aceita.', 'citação': 'citado em delação/CPI = menção em depoimento ou apuração; não é investigação formal contra a pessoa.' }
const STBADGE = { 'ativo': ['⚠️', 'alerta'], 'encerrado': ['ℹ️', 'info'], 'citação': ['📎', 'info'] }
function seloHTML(pid) {
  const reg = (D.registros[S.casa] || {})[pid]
  const rodape = `<div class="sm" style="margin-top:8px;color:var(--mut)">Política: só fatos processuais com fonte, desfecho explícito e versão da defesa quando publicada — sem juízo de valor. Absolvição e arquivamento também são fatos. <a href="mailto:wncgui@gmail.com?subject=Bússola do Voto — correção de registro" >Encontrou erro? Peça correção</a>.</div>`
  if (reg && Array.isArray(reg.itens)) {
    const ativo = reg.itens.some(x => x.st === 'ativo')
    return `<div class="tcell wide"><b class="lbl">Registros públicos — histórico (verificado em ${esc((reg.verificadoEm || '').split('-').reverse().join('/'))})</b>
      ${ativo ? '<span class="selo alerta">⚠️ há processo ativo</span>' : '<span class="selo info">ℹ️ só registros encerrados</span>'}
      <div style="margin-top:8px;display:flex;flex-direction:column;gap:7px">${reg.itens.map(x => { const b = STBADGE[x.st] || ['·', 'info']; return `<div class="sm" style="border-left:3px solid ${x.st === 'ativo' ? 'var(--warm)' : 'var(--line)'};padding-left:10px"><b>${b[0]} ${esc(x.tipo)}${x.ano ? ' · ' + esc(String(x.ano)) : ''}</b> — ${esc(x.desc)}${x.desfecho ? ` <b>Desfecho:</b> ${esc(x.desfecho)}` : ''}${x.defesa ? ` <i>Defesa: ${esc(x.defesa)}</i>` : ''} <a href="${esc(x.fonte)}" target="_blank" rel="noopener">fonte ↗</a></div>` }).join('')}</div>${rodape}</div>`
  }
  if (reg && reg.status === 'histórico') return `<div class="tcell"><b class="lbl">Registros públicos (verificado em ${esc(reg.verificadoEm.split('-').reverse().join('/'))})</b><span class="selo info">ℹ️ histórico — casos encerrados</span><div class="sm" style="margin-top:6px">${esc(reg.desc)}</div><div class="sm" style="margin-top:4px">Casos encerrados (arquivamento, prescrição ou absolvição) também são fatos públicos. <a href="${esc(reg.fonte)}" target="_blank" rel="noopener">fonte ↗</a></div>${rodape}</div>`
  if (reg) return `<div class="tcell"><b class="lbl">Registros públicos (verificado em ${esc(reg.verificadoEm.split('-').reverse().join('/'))})</b><span class="selo alerta">⚠️ ${esc(reg.status)}</span><div class="sm" style="margin-top:6px">${esc(reg.desc)}</div><div class="sm" style="margin-top:4px;color:var(--mut)">${GLOSS[reg.status] || ''} <a href="${esc(reg.fonte)}" target="_blank" rel="noopener">fonte ↗</a></div>${rodape}</div>`
  return `<div class="tcell"><b class="lbl">Registros públicos (verificado em 09/06/2026)</b><span class="selo ok">✅ sem processos ativos encontrados</span><div class="sm" style="margin-top:6px">nas fontes verificadas (réu, condenação, denúncia ou inquérito no STF/STJ); não é atestado de idoneidade — processos sob sigilo não aparecem e a varredura de históricos é gradual.</div>${rodape}</div>`
}
...
function results() {
  ...
  <div class="fade">
    ...
    <div class="meth">
      ...
      <p>· <b>Registros públicos</b>: ⚠️ = processo ATIVO (réu, condenação, denúncia pendente ou inquérito formal no STF/STJ), confirmado em fonte oficial ou grande imprensa citando o caso. ℹ️ = histórico relevante já ENCERRADO (arquivado, prescrito ou absolvição), com desfecho explícito — arquivamento também é fato público. ✅ = nada ativo encontrado nas fontes verificadas em 09/06/2026; não é atestado de idoneidade. No Senado a varredura cobre ativos e históricos; na Câmara, históricos ainda parcialmente (expansão gradual). Política editorial: cada item é um fato processual datado com fonte, desfecho explícito (arquivado/absolvido/em curso) e versão da defesa quando publicada; citação em delação é categoria própria, distinta de investigação formal; identificação rígida da pessoa (homônimos e parentes nunca se misturam); correções com fonte são bem-vindas pelo link em cada cartão. Este site não acusa ninguém: relata e linka.</p>
...
function card(r, i, cfg, c) {
  ...
  const regAll = (D.registros[S.casa] || {})[r.p.id]; const reg = regAll && (Array.isArray(regAll.itens) ? regAll.itens.some(x => x.st === 'ativo') : regAll.status !== 'histórico') ? regAll : null
